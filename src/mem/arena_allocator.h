...
class ArenaAllocator {
 public:
  ...
  // Reverts the allocation position to the one before the failed append.
  // Pos should be obtained from current_position() before the failed append.
  // Rollback is only supported within the same block.
  inline void rollback(const BlockPosition& pos) {
    dcheck(block_);
    dcheck_lt(pos.block_id, block_count_);
    // Only rollback within the same block is supported.
    dcheck_eq(pos.block_id, block_count_ - 1);
    dcheck_le(pos.offset, block_[pos.block_id]->used);
    block_[pos.block_id]->used = pos.offset;
  }
