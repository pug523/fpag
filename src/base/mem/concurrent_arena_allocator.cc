...
#include "base/mem/concurrent_arena_allocator.h"
...
ConcurrentArenaAllocator::ConcurrentArenaAllocator(
...
ConcurrentArenaAllocator& ConcurrentArenaAllocator::operator=(
    ConcurrentArenaAllocator&& other) noexcept {
  if (this != &other) {
    reset();
    block_ = other.block_;
...
void ConcurrentArenaAllocator::reset() {
  if (!block_) [[unlikely]] {
    return;
  }
...
void ConcurrentArenaAllocator::alloc_new_block(usize size,
                                               bool use_huge_pages) {
  dcheck_lt_msg(block_count_, kMaxBlocks,
                "ConcurrentArenaAllocator out of memory (too many blocks)");
...
void ConcurrentArenaAllocator::init_block_map(Block* first_block) {
  dcheck(first_block);
...
void* ConcurrentArenaAllocator::try_alloc_from_block(usize size, usize align) {
...
inline void ConcurrentArenaAllocator::write_block_pos(void* ptr,
                                                      BlockPosition* pos) {
  dcheck(ptr);
  dcheck(pos);
